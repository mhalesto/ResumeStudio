...
private final class ResumePDFLayout {
    ...
    private func drawPrimaryHeader() {
        ...
        NSAttributedString(string: firstName, attributes: firstAttributes).draw(at: CGPoint(x: margin, y: 27))
        ...
        for value in [document.personal.phone, document.personal.email] {
            ...
            NSAttributedString(string: clean, attributes: attributes).draw(at: CGPoint(x: contactX, y: contactY))
    ...
    private func drawCompetencies() {
        ...
        for index in stride(from: 0, to: items.count, by: 2) {
            ...
            let leftHeight = measuredHeight(left, width: columnWidth - 14, font: regularFont(8.8), lineHeight: 11)
            let rightHeight = right.map {
    ...
    private func drawExperienceEntry(_ entry: ExperienceEntry) {
        ...
        let metaHeight = measuredHeight(metadata, width: contentWidth, font: mediumFont(8.6), lineHeight: 11)
        let firstBulletHeight = highlights.first.map {
    ...
    private func drawEducation() {
        ...
        for entry in entries {
            ...
            let detailsHeight = details.isEmpty ? 0 : measuredHeight(
                details,
                width: contentWidth,
    ...
    private func drawReferences() {
        ...
        for index in stride(from: 0, to: references.count, by: 2) {
            ...
            drawReferenceCard(references[index], x: margin, y: cursorY, width: cardWidth, height: cardHeight)
    ...
    private func drawReferenceCard(
        ...
        lightGray.setFill()
        rendererContext.cgContext.fill(CGRect(x: x, y: y, width: width, height: height))
        accent.setFill()
        rendererContext.cgContext.fill(CGRect(x: x, y: y, width: 4, height: height))

        let insetX = x + 16
        let textWidth = width - 27
        drawText(
            reference.name,
    ...
    private func drawBullet(_ text: String, height: CGFloat) {
        accent.setFill()
        rendererContext.cgContext.fillEllipse(in: CGRect(x: margin + 3, y: cursorY + 4, width: 3.2, height: 3.2))
        drawText(
            text,
    ...
    private func drawCompactBullet(_ text: String, x: CGFloat, y: CGFloat, width: CGFloat, height: CGFloat) {
        accent.setFill()
